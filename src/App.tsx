...
function NavBar({ active = "home" }) {
  const items = [
    ...
    { k: "sidegames", ic: "🎯", l: "Side Games" },
    {
      ...
      ic: "$",
      l: "Skins",
      icStyle: { color: "#c9a84c", fontWeight: 700, fontFamily: "serif" },
    },
    { k: "chat", ic: "💬", l: "Chat" },
  ];
  return (
...
function buildShareMessage(winnerName, winnerScore, tripName) {
  return [
    "🏆 " + tripName + " Results",
    "",
    winnerName + " wins with " + winnerScore + " pts!",
    "",
    "We just ran this on Teein' It Up — live scoring, side comps, leaderboard all done automatically.",
    "",
    "Try it yourself:",
    "https://z89rc5.csb.app/",
  ].join("\n");
}
...
function doShare(msg, onCopied) {
  ...
  if (navigator.share) {
    navigator
      .share({ title: "Teein' It Up Demo", text: msg })
      .then(function () {
        if (onCopied) onCopied();
      })
      .catch(function () {
...
function WinnerOverlay({ winner, sideW, onClose }) {
  ...
  const MSG_ORGANISER = [
    "🏌️ Try this golf trip demo",
    "",
    "This is Teein' It Up — a simple way to run golf trips with live scoring, leaderboards, side comps and automatic winners.",
    "",
    "It takes 60 seconds to test drive.",
    "",
    "Try it here:",
    "https://z89rc5.csb.app/",
  ].join("\n");
  const MSG_PLAYERS = [
    "🏌️ Before the trip — try this quick demo",
    "",
    "This is how we'll run scoring, leaderboards and side comps on Teein' It Up.",
    "",
    "Have a quick test drive so everyone knows how it works before the first tee.",
    "",
    "Try it here:",
    "https://z89rc5.csb.app/",
  ].join("\n");
  const MSG_RESULTS = buildShareMessage(
    winner.name,
    winner.total,
    mockTrip.name
  );
    ...
    >
      ...
      >
        ...
        {showSharePanel && (
          ...
          >
            {[
              { label: "Share with another organiser", msg: MSG_ORGANISER },
              { label: "Send to players before the trip", msg: MSG_PLAYERS },
              { label: "Share final results", msg: MSG_RESULTS },
            ].map((opt, i, arr) => (
              <div
                key={i}
                onClick={(e) => {
              ...
              >
                ...
                <span style={{ fontSize: 12, color: "rgba(201,168,76,.5)" }}>
                  →
                </span>
              </div>
            ))}
        ...
        {!showSharePanel && (
          <div
            style={{
              ...T.body,
              textAlign: "center",
              color: "rgba(245,230,184,.48)",
              fontSize: 11,
              marginTop: 7,
              opacity: vis ? 1 : 0,
              transition: "opacity .5s .78s",
            }}
          >
            Send this to your group — they can try it before your trip
          </div>
        )}
...
function LeaderboardScreen({ userScores, scRes, cfg, dailyHcps, finalBoard }) {
  ...
  function ctaShareText(msg) {
    var _b = board && board.length > 0 ? board[0] : null;
    doShare(msg, function () {
      setCtaShareToast(true);
      setShowCtaPanel(false);
      setTimeout(function () {
        setCtaShareToast(false);
      }, 2800);
    });
    setShowCtaPanel(false);
  }
          ...
          <div style={{ padding: "22px 22px 18px", textAlign: "center" }}>
            ...
            <div
              ...
              style={{
                ...
                textDecorationColor: "rgba(201,168,76,.35)",
              }}
            >
              Share this demo with your group →
            </div>
            {showCtaPanel && (
              <div
                style={{
              ...
              >
                {(() => {
                  ...
                  return [
                    ...
                    {
                      label: "Send to players before the trip",
                      msg: CTA_MSG_PLY,
                    },
                    { label: "Share final results", msg: _res },
                  ].map((opt, i, arr) => (
                    <div
                      key={i}
                      onClick={() => ctaShareText(opt.msg)}
                      style={{
                        padding: "11px 16px",
                        borderBottom:
                          i < arr.length - 1
                    ...
                    >
                      ...
                      <span
                        style={{ fontSize: 12, color: "rgba(201,168,76,.5)" }}
                      >
                        →
                      </span>
                    </div>
